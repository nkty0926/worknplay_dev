...
if (isset($_POST['action']) && $_POST['action'] == 'appr' && isset($_POST['no']) && !empty($_POST['no'])) {
	$query = "update story_profile set appr = (appr + 1) % 2 where no = :no";
	...
	$query = "update story_profile set top = (top + 1) % 2 where no = :no";
	$stmt = $DB->conn->prepare($query);
	$stmt->bindParam(":no", $_POST['no']);
	$stmt->execute();
	$stmt->closeCursor();
	...
	$articles = $DB->conn->query("select story_profile.no, story_profile.date, story_profile.mod, story_profile.publ, story_profile.appr, story_profile.top, story_profile.hits, story_profile.nickname, member.email from story_profile left join member on story_profile.member = member.no" . (isset($_GET['top'])?" where story_profile.top = 1":""))->fetchAll(PDO::FETCH_ASSOC);
}
...
$(document).on('change', 'input[type="checkbox"][name="appr"]', function(){
	$.ajax({ type: 'post', url: '/ADMIN/Blogs/Profile', data: 'action=' + $(this).attr('name') + '&no=' + $(this).val() });
...
<?php foreach($articles as $article){ ?>
				<tr>
					...
					<td><a class="btn-link" href="/Blogs/Detail/Profile/<?= $article['no'] ?>" target="_blank"><?= $article['nickname'] ?></a></td>
					...
					<td><?= $article['hits'] ?></td>
					<td><a class="btn-link" data-toggle="action" data-action="delete" data-table="story_profile" data-pk="<?= $article['no'] ?>">삭제</a></td>
				</tr>
<?php } ?>
			</tbody>
		</table>
		<a class="btn btn-link float-right" href="/ADMIN/Blogs/Profile<?= isset($_GET['top'])?'':'?top=true' ?>"><?= isset($_GET['top'])?'all':'featured' ?> profile</a>
	</section>