...
if (isset($_POST['action']) && $_POST['action'] == 'appr' && isset($_POST['no']) && !empty($_POST['no'])) {
	$query = "update work_company set appr = (appr + 1) % 2 where no = :no";
	...
	$query = "update work_company set top = (top + 1) % 2 where no = :no";
	$stmt = $DB->conn->prepare($query);
	$stmt->bindParam(":no", $_POST['no']);
	$stmt->execute();
	$stmt->closeCursor();
	...
	$articles = $DB->conn->query("select work_company.no, work_company.date, work_company.mod, work_company.publ, work_company.appr, work_company.top, work_company.hits, work_company.name, work_company.name_kor, member.email from work_company left join member on work_company.member = member.no" . (isset($_GET['top'])?" where work_company.top = 1":""))->fetchAll(PDO::FETCH_ASSOC);
}
...
$(document).on('change', 'input[type="checkbox"][name="appr"]', function(){
	$.ajax({ type: 'post', url: '/ADMIN/Work/Company', data: 'action=' + $(this).attr('name') + '&no=' + $(this).val() });
...
<?php foreach($articles as $article){ ?>
				<tr>
					...
					<td><a class="btn-link" href="/Work/Detail/Company/<?= $article['no'] ?>" target="_blank"><?= $article['name'] ?><?= $article['name_kor']?' <small>(' . $article['name_kor'] . ')</small>':'' ?></a></td>
					...
					<td><?= $article['hits'] ?></td>
					<td><a class="btn-link" data-toggle="action" data-action="delete" data-table="work_company" data-pk="<?= $article['no'] ?>">삭제</a></td>
				</tr>
<?php } ?>
			</tbody>
		</table>
		<a class="btn btn-link float-right" href="/ADMIN/Work/Company<?= isset($_GET['top'])?'':'?top=true' ?>"><?= isset($_GET['top'])?'all':'featured' ?> company</a>
	</section>