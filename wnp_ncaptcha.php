...
if(empty($_REQUEST['ncaptcha_value'])){
  ...
  if($status_code == 200) {
    // echo $response;
    $fp = fopen("/data/worknplay/www/ncaptcha/captcha.jpg", "w+");
    fwrite($fp, $response);
    fclose($fp);
?>
<div style="width:200px;">
    <img src='/ncaptcha/captcha.jpg?date=<?= date('YmdHis').$key ?>' />
	<input type="text" name="ncaptcha_value" value="" style="width:100%;" required />
</div>
<?php
  } else {
    echo "Error 내용:".$response;
